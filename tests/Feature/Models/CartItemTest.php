...
it('adding the same product twice creates two separate rows', function (): void {
    $cart = Cart::factory()->create();
    $product = Product::factory()->create();
...
it('items() returns only non-deleted rows by default', function (): void {
    $cart = Cart::factory()->create();
    $kept = $cart->items()->create(['product_id' => Product::factory()->create()->id]);
    $removed = $cart->items()->create(['product_id' => Product::factory()->create()->id]);
    $removed->delete();

    $visible = $cart->items()->get();

    expect($visible)->toHaveCount(1)
        ->and($visible->first()->id)->toBe($kept->id);
});
...
it('has a 26-char ULID and getRouteKeyName returns ulid', function (): void {
    ...
    expect($item->ulid)->toBeString()->toHaveLength(26)
        ->and($item->getRouteKeyName())->toBe('ulid');
});
