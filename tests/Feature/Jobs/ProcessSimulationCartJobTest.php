...
it('processes a cart and creates simulation records', function (): void {
    ...
    $job = new ProcessSimulationCartJob(
    ...
    $this->assertDatabaseHas('simulated_carts', [
    ...
    $simulatedCartItem = SimulatedCartItem::query()
        ->where('cart_item_id', $cartItem->id)
        ->firstOrFail();

    $this->assertDatabaseHas('promotion_redemptions', [
        'promotion_stack_id' => $stack->id,
        'redeemable_type' => SimulatedCartItem::class,
