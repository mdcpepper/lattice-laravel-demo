...
class CartSavingDistributionChartWidget extends ChartWidget
{
    ...
    {
        return ($this->record->processed_carts / $this->record->total_carts) * 100;
    }
    ...
    protected function getData(): array
    {
        ...
        $buckets = $this->record->simulatedCarts()
            ->whereColumn('total', '<', 'subtotal')
            ->selectRaw('FLOOR((subtotal - total) / 100) as bucket, COUNT(*) as count')
            ->groupBy('bucket')
            ->orderBy('bucket')
            ->pluck('count', 'bucket');
